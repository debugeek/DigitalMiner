...
extension Data {
    ...
    func uint32() -> UInt32? {
        return withUnsafeBytes({ (pointer: UnsafePointer<UInt32>) -> UInt32? in
            if MemoryLayout<UInt32>.size != self.count {
                return nil
            }
    ...
    func uint64() -> UInt64? {
        return withUnsafeBytes({ (pointer: UnsafePointer<UInt64>) -> UInt64? in
            if MemoryLayout<UInt64>.size != self.count {
                return nil
            }
            return pointer.pointee
        })
...
extension Data {
    ...
    func write(toStream stream: OutputStream) {
        self.withUnsafeBytes { (buffer) in
            write(buffer: buffer, count: self.count, toStream: stream)
        }
    }

    private func write(buffer: UnsafePointer<UInt8>, count: Int, toStream stream: OutputStream) {
        var bytesToWrite = count
        while bytesToWrite > 0, stream.hasSpaceAvailable {
            let bytes = stream.write(buffer, maxLength: bytesToWrite)
            bytesToWrite -= bytes
        }
    }
